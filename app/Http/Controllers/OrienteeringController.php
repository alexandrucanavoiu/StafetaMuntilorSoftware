...
class OrienteeringController extends Controller
{
    ...
    public function index($stageid, $id)
    {
        ...
        if($category == null) {
            ...
            $teams = Team::where('stage_id', $stageid)->with('orienteering')->where('category_id', $id)->get();
            $number = 1;
            return view('orienteering.index',compact('category', 'teams', 'number', 'stageid'));
        }
    }
    ...
    public function edit($stageid, $categoryid, $teamid, Request $request)
    {
        if( $request->ajax() )
        {
            ...
            if($team == null || $category == null) {
                ...
                return redirect()->route('orienteering.index', [$stageid, $categoryid])->with($notification);
            } else {
                $orienteering_result = [];
                $orienteering_result['stage_id'] = $stageid;
                $orienteering = Orienteering::where('stage_id', $stageid)->where('team_id', $teamid)->first();
                ...
                if($orienteering == null){
                    ...
                    return response()->json( [
                        ...
                        'view_content' => view('orienteering.edit', ['team' => $team, 'category' => $category, 'orienteering_result' => $orienteering_result, 'stageid' => $stageid])->render()
                    ] );
                } else {
        ...
        if( $request->ajax() )
        {
            ...
            $category = Category::FindOrFail($categoryid);
                ...
                if($team == null || $category == null ) {
                    ...
                    $data = $request->only(['start_time', 'finish_time', 'total_time', 'missed_posts', 'abandon', 'created_at', 'updated_at', 'team_id', 'stage_id']);
                    ...
                    $orienteering = Orienteering::where('stage_id', $stageid)->where('team_id', $team->id)->first();
                        if($orienteering == null) {
                            // If the team is not in orienteering table insert the data
                            Orienteering::create($data);
                            $ajax_redirect_url = route('orienteering.index', [$stageid, $categoryid]);
                            $ajax_message_response = "Datele au fost adaugate.";
                            $ajax_title_response = "Felicitări!";
                            $ajax_status_response = "success";
                            return response()->json(['ajax_redirect_url' => $ajax_redirect_url, 'ajax_status_response' => $ajax_status_response, 'ajax_title_response' => $ajax_title_response, 'ajax_message_response' => $ajax_message_response, 'stage_id' => $stageid], 200);
                        } else {
